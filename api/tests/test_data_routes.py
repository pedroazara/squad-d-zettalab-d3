...
def test_get_regions_maps_payload(client, monkeypatch):
    monkeypatch.setattr(
        regions_route,
...
def test_get_risk_ok(client, monkeypatch):
    monkeypatch.setattr(
        risk_route,
        "list_risk_payloads",
    ...
    response = client.get("/risk?region_id=1")
    assert response.status_code == 200
    assert response.json()[0]["risco"] == "alto"


def test_get_risk_region_not_found(client, monkeypatch):
    monkeypatch.setattr(risk_route, "list_risk_payloads", lambda db, region_id, ano_mes, limit, offset: [])

    response = client.get("/risk?region_id=999")
    assert response.status_code == 404


def test_get_climate_ok(client, monkeypatch):
    monkeypatch.setattr(
        climate_route,
        "list_climate_items",
    ...
    response = client.get("/climate?ano=2024&mes=7")
    assert response.status_code == 200
    assert response.json()[0]["estacao_codigo"] == "A001"


def test_get_fires_and_points(client, monkeypatch):
    monkeypatch.setattr(
        fires_route,
        "list_fire_items",
    ...
    fires_response = client.get("/fires?ano_mes=2024-07")
    points_response = client.get("/fires/points?ano_mes=2024-07")
